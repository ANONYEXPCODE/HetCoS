...
import numpy as np
from my_lib.util.code_parser.astor import MyAstor
# from my_lib.util.code_parser.py_parser import py2ast_sitter,tokenize_python
...
from my_lib.util.code_parser.code_tokenizer import (CompoundWordSplitter,
                                                    tokenize_code_str)
from my_lib.util.nl_parser.en_parser import EnWordCheck, punc_str
...
def make_rev_dic(train_raw_data_path,valid_raw_data_path,test_raw_data_path,tech_term_path,rev_dic_path,noise_token_path):
    ...
    parser=MySitter(lan='java',lemmatize=False,lower=False,ast_intact=True,seg_attr=True,rev_dic=None,user_words=USER_WORDS)    #这里lower=False！！
    all_tokens=[]
...
def _truncate_ast_by_code_and_layout(parser,start_code_token_id,end_code_token_id,max_graph_size=np.inf,renew_pos=True,):    
    ...
    if start_code_token_id>0 or end_code_token_id<len(code_token_ids):
        ...
        while len(child_node_ids)>0:
            ...
            wids=np.where(_parser_dict['ast_edges'][0, :]==child_node_ids[:,None])   #超找当前child_node_ids在ast_edges[0, :]中的索引
...
def tokenize_raw_data(raw_data_path, token_data_path, rev_dic_path,
    ...
    for item in pbar:
        ...
        if "ERROR" in parser.ast_nodes:
            error_item += 1
            pbar.set_description("[The {}-th erroneously parsed item]".format(error_item))
        # parser_dict=_truncate_ast(parser,max_code_len=max_code_len,max_graph_size=max_graph_size)
        parser_dict=_truncate_ast_by_code_and_layout(parser,3,min(len(parser.code_tokens)-1,3+max_code_len),max_graph_size=max_graph_size,renew_pos=True)
        
        assert parser_dict['code_layout_edges'].dtype==np.int64
        # assert parser_dict['ast_sibling_edges'].dtype==np.int64
        layout_nodes=[]
        # layout_node_in_code_poses=[]
        ...
        # code_token_ids=sorted(np.unique(code_token_edges))
        # root_node_ids=set(layout_edges[1,:])-set(layout_edges[0,:])
        # assert len(root_node_ids)==1
        # root_node_id=list(root_node_ids)[0]
        # first_code_token_ids=set(code_token_edges[0,:])-set(code_token_edges[1,:])
        # assert len(first_code_token_ids)==1
        # first_code_token_id=list(first_code_token_ids)[0]
        # last_code_token_ids=set(code_token_edges[1,:])-set(code_token_edges[0,:])
        # assert len(last_code_token_ids)==1
        # last_code_token_id=list(last_code_token_ids)[0]
        # assert first_code_token_id<=last_code_token_id
        # add_layout_edges=np.array([[root_node_id]*len(code_token_ids),code_token_ids])
        # add_code_token_edges=np.array([[last_code_token_id],[first_code_token_id]])
        ...
        item['graph'] = {'nodes': str(layout_nodes),
                    ...
                    #    'add_base_edges':str(add_layout_edges.tolist()),
                       'sibling_edges':str(layout_sibling_edges.tolist()),
                       'dfg_edges':str(DFG_edges.tolist()),
                       'code_edges':str(code_token_edges.tolist()),
                    #    'add_code_edges':str(add_code_token_edges.tolist()),
                    #    'node_in_code_poses':str(['({},{})'.format(pos[0], pos[1]) for pos in layout_node_in_code_poses])
                       }
...
def build_avail_data(token_data_path,
    ...
    for i,item in enumerate(pbar):
        ...
        # base_child2father_edges = np.concatenate([np.array(eval(item['graph']['base_edges'])),
        #                                     np.array(eval(item['graph']['add_base_edges']))],axis=-1)
        ...
        # code_prev2next_edges = np.concatenate([np.array(eval(item['graph']['code_edges'])),
        #                                         np.array(eval(item['graph']['add_code_edges']))],axis=-1)
        ...
        # graph_node_in_code_poses=[eval(pos) for pos in list(eval(item['graph']['node_in_code_poses']))] #所有node的code pos
        ...
        # graph_node_in_code_mposes,graph_node_in_code_nposes=zip(*(graph_node_in_code_poses))
        # graph_node_in_code_mpos_ids=[code_pos_w2i['m'].get(pos,m_pos_unk_idx) for pos in graph_node_in_code_mposes]
        # graph_node_in_code_npos_ids=[code_pos_w2i['n'].get(pos,n_pos_unk_idx) for pos in graph_node_in_code_nposes]
        
        code_graph={'nodes': graph_node_ids,
                    # 'node_in_code_poses':np.array([graph_node_in_code_mpos_ids,graph_node_in_code_npos_ids]),
